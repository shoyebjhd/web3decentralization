...
/**
 * Curated 404 â†’ canonical aliases (exact path, no guesswork).
 *
...
/**
 * --------------------------------------------------------------------------
 * OG images + chain logos + tool icons.
 * Static 1200x630 PNGs live in /wp-content/uploads/og/; inc/og-dynamic.php
 * serves them (or renders via GD for anything missing). Rank Math emits the
 * og:image meta; the fallback <meta> below also fires when Rank Math is off.
 * --------------------------------------------------------------------------
 */
require get_template_directory() . '/inc/og-dynamic.php';

/**
 * Resolve an og file key ("btc", "tool-nakamoto-coefficient", "crypto-tax-guide")
 * to its published content object.
 */
function w3d_og_lookup( $key ) {
	$name  = ( 0 === strpos( $key, 'tool-' ) ) ? substr( $key, 5 ) : $key;
	$types = ( 0 === strpos( $key, 'tool-' ) ) ? array( 'w3d_tool' ) : array( 'chain', 'post', 'course' );

	$post = get_page_by_path( $name, OBJECT, $types );
	if ( ! $post ) {
		$post = get_page_by_path( $name, OBJECT ); // any post type
	}
...
/**
 * Registry used by inc/og-dynamic.php to know title/badge/glyph/kind.
 */
function w3d_og_registry( $key ) {
	$post = w3d_og_lookup( $key );
	if ( ! $post ) {
		return false;
	}
	...
	$meta = array(
		...
		'glyph'    => '',
		'kind'     => 'plain',
		'file_key' => ( 'w3d_tool' === $pt ? 'tool-' : '' ) . $post->post_name,
	);

	if ( 'chain' === $pt ) {
		$symbol      = get_field( 'symbol', $post->ID );
		$name        = get_field( 'name', $post->ID ) ? get_field( 'name', $post->ID ) : $post->post_title;
		$score       = get_field( 'score_total', $post->ID );
		$has_score   = ( '' !== $score && null !== $score );
		$meta['title'] = $name . ( $has_score ? ' Decentralization Score: ' . $score : ' Decentralization Score' );
		$meta['badge'] = $has_score ? sprintf( 'Score %s/100 • 4-Pillar Model', $score ) : 'Preliminary • 4-Pillar Model';
		$meta['glyph'] = $symbol ? $symbol : strtoupper( $post->post_title );
		$meta['kind']  = 'chain';
	} elseif ( 'w3d_tool' === $pt ) {
		$meta['title'] = preg_replace( '/\s*[-–—]\s*w3d terminal$/i', '', $post->post_title );
		$meta['badge'] = '15 Tools • MIT Licensed';
		$meta['glyph'] = $post->post_name;
		$meta['kind']  = 'tool';
	} elseif ( 'course' === $pt ) {
		$meta['badge'] = 'Free Course • Certificate';
		$meta['kind']  = 'plain';
	} else {
		$meta['badge'] = 'Open-Source Hub • MIT Licensed';
		$meta['kind']  = 'plain';
	}
...
/**
 * URL of the static (or GD-learned) og image for a key.
 */
function w3d_og_url_from_key( $key ) {
	$meta = w3d_og_registry( $key );
	if ( ! $meta ) {
		return '';
	}
	return wp_upload_dir()['baseurl'] . '/og/og-' . $meta['file_key'] . '.png';
}

function w3d_og_current_key() {
	...
	if ( ! in_array( $post->post_type, array( 'chain', 'w3d_tool', 'post', 'course' ), true ) ) {
		return '';
	}
	return ( 'w3d_tool' === $post->post_type ? 'tool-' : '' ) . $post->post_name;
}

// Single source of truth for the og:image meta under Rank Math.
add_filter( 'rank_math/opengraph/facebook/og_image', function ( $img ) {
	$key = w3d_og_current_key();
	if ( ! $key ) {
		return $img;
	}
	$url = w3d_og_url_from_key( $key );
	return $url ? $url : $img;
}, 99 );

// Same meta emitted directly when Rank Math is inactive/unloaded.
...
// /og/<file>.png -> dynamic endpoint (static file served when present).
add_action( 'init', function () {
	add_rewrite_rule( '^og/([^/]+\.png)$', 'index.php?w3d_og=$matches[1]', 'top' );
} );
...
add_action( 'template_redirect', 'w3d_og_endpoint' );
