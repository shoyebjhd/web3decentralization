...
- **Infrastructure:** Polygon PoS runs a well-documented validator set with
  delegation, off the heels of a historical operator-concentration event; the
  CDK chains inherit Ethereum's security.
- **Capital:** the POL ecosystem and validator stake distribution are broad but
  carry real foundation-assigned weight.
- **Governance:** the shift from MATIC to POL and the CDK governance controls
  are where "who decides" actually lives.
- **Software:** client diversity on PoS has improved since the network-split
  era, but tracking *which* client each validator runs is still immature.
...
- [Interactive audit & scorecard](/terminal/chains/polygon)
- [Run Polygon in the terminal](/terminal/?tool=nakamoto-coefficient&chain=polygon)
