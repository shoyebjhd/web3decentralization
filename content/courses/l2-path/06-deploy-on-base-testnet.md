...
order: 6
course: l2-path
description: "You already read a storage contract in Smart Contracts 101."
---
...
Deployment sends a transaction whose *payload creates* a contract; the network returns its address. On testnets the mechanics are identical to mainnet â€” same tools, same explorers, free fake money.
...
3. Call `store(42)`, then `retrieve()` â€” read the values back.
4. Find your contract on the Base Sepolia explorer; verify the creation transaction and try reading it there too.

## Safety checklist

- Testnet wallet only â€” never connect a mainnet wallet to experimental tooling.
...
- [Faucet](glossary/faucet.md) Â· [Smart Contract](glossary/smart-contract.md) Â· [Base Chain](glossary/base-chain.md)
