...
order: 10
course: l2-path
description: "Restake â†’ delegate to operators â†’ operators run AVS software â†’ rewards stack, slashing stacks harder."
---

**Objective:** understand restaking by walking through it hands-on â€” without risking a wei of real stake.
...
Restake â†’ delegate to operators â†’ operators run AVS software â†’ rewards stack, slashing stacks harder. [LRTs](glossary/lrt-token.md) add liquidity on top. Every layer adds yield and a new failure mode in equal measure.
...
4. Simulate (on paper) a 10% slash event on a restaked position also looped through an LRT â€” trace every loss layer.
...
- [EigenLayer](glossary/eigenlayer.md) Â· [Restaking](glossary/restaking.md) Â· [LRT Token](glossary/lrt-token.md)
